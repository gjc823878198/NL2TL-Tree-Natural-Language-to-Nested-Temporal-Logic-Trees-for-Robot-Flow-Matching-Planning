...
def main():
    ...
    ax.set_xlim(0, 162); ax.set_ylim(15, 81); ax.axis("off")

    def box(x, y, w, h, t, fc, ec, fs=10.5, bold=True, z=3):
    ...
    def tag(xc, ytop, lines, w=27):
        h = 2.2*len(lines) + 1.4
        box(xc - w/2, ytop - h, w, h, "\n".join(lines), MECH, MECH_E, fs=8.6,
            bold=True)

    # ---- banner ----
    ax.text(81, 77.0, "NL2TL-Tree: fully zero-shot   "
            "(LLM and flow-matching planner are FROZEN — no fine-tuning)",
            ha="center", fontsize=14.5, fontweight="bold", color="#2c3e6b")
    ...
    ax.text(12, 46.5, "Natural-language\ntask  (phone)", ha="center",
            va="center", fontsize=10.2, fontweight="bold", color="#16213a", zorder=4)
    ax.text(12, 41.0, "“visit A, then B;\nalways keep safe”", ha="center",
            va="center", fontsize=8.2, style="italic", fontweight="bold",
    ...
    ax.text(78, 65.5, "Zero-shot pipeline  (nothing trained)", ha="center",
            fontsize=12.0, fontweight="bold", color="#2c3e6b")

    sy, sh = 37, 13
    ymid = sy + sh/2
    # stage 1: parse
    box(29, sy, 27, sh, "(1) Frozen LLM parse\n$\\rightarrow$ nested STL tree",
        ST, ST_E, fs=10.5, bold=True)
    tag(42.5, sy-4.0, ["self-consistency vote (#1)",
                       "round-trip scope refine (#4)",
                       "robustness-feedback self-correct"])
    # stage 2: render
    box(60, sy, 24, sh, "(2) Render robustness\nSTL $\\varphi$, diff.\\ $\\rho$",
        ST, ST_E, fs=10.5, bold=True)
    tag(72, sy-4.0, ["operator norm.: $\\rightarrow,\\leftrightarrow\\Rightarrow$ basis",
                     "sensor-grounded  $G\\,\\neg$unsafe"])
    # stage 3: flow-matching planner (with denoising row)
    box(88, sy, 39, sh, "", ST, ST_E, z=3)
    ax.text(107.5, sy+sh-2.6, "(3) Frozen flow-matching planner (TeLoGraF)",
            ha="center", fontsize=10.0, fontweight="bold", color="#16213a", zorder=5)
    # GNN encoder chip
    box(90, sy+3.0, 9, 5.0, "GNN\nencoder", "#d7e3f2", ST_E, fs=7.6, z=5)
    # denoising / flow row x_T -> x_0
    xs = [104, 110, 116, 122]; yrow = sy+5.5
    labels = ["$x_T$", "$x_t$", "$\\cdots$", "$x_0$"]
    for i, (xx, lb) in enumerate(zip(xs, labels)):
        ax.add_patch(Circle((xx, yrow), 1.9, facecolor="#cfe0d2",
                     edgecolor=GATE_E, lw=1.1, zorder=5))
        ax.text(xx, yrow, lb, ha="center", va="center", fontsize=8.5,
                fontweight="bold", zorder=6)
        if i < len(xs)-1:
            arr(xx+1.9, yrow, xs[i+1]-1.9, yrow, ec=GATE_E, lw=1.1)
    arr(99, yrow, 102.1, yrow, ec=ST_E, lw=1.1)
    ax.text(113, sy+1.7, "flow sampling $+$ STLCG $\\nabla\\rho$ guidance",
            ha="center", fontsize=8.2, fontweight="bold", color="#16213a", zorder=6)
    tag(107.5, sy-4.0, ["nested-tree decomposition",
                        "online $\\rho$ monitor $+$ replan"], w=33)

    # inter-stage arrows
    arr(56, ymid, 60, ymid); arr(84, ymid, 88, ymid)

    # feasibility self-check -> A* fallback (above stage 3)
    box(96, 55, 23, 5.5, "feasibility self-check (best-of-$N$)", GATE, GATE_E, fs=8.6, z=4)
    arr(107.5, sy+sh, 107.5, 55, ec=GATE_E, lw=1.1)
    box(122, 55, 6.5, 5.5, "A*", ALT, ALT_E, fs=9.5, bold=True, z=4)
    arr(119, 57.75, 122, 57.75, ec=ALT_E, lw=1.2)
    ax.text(120.5, 62.0, "OOD", ha="center", fontsize=7.6, fontweight="bold",
            color="#b03a2e")
    ...
    ax.text(146, 47, "Output", ha="center", fontsize=10.5, fontweight="bold",
            color="#7a4d1c", zorder=4)
    ax.text(146, 42.0, "STL-satisfying\ntrajectory $\\tau$\n"
